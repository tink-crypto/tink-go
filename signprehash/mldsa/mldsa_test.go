...
const (
	mldsaTestSeed65Hex = "7C9935A0B07694AA0C6D10E4DB6B1ADD2FD81A25CCB148032DCD739936737F2D"
)

func TestRegisterPrimitiveConstructor(t *testing.T) {
	cb := config.NewBuilder()
	if err := mldsa.RegisterPrimitiveConstructor(cb, internalapi.Token{}); err != nil {
		t.Fatalf("mldsa.RegisterPrimitiveConstructor() err = %v, want nil", err)
	}
	c := cb.Build()
	...
	seedBytes, _ := hex.DecodeString(mldsaTestSeed65Hex)
